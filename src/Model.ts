...
import type { RecordPaths, GetPathValue } from "./types"
...
export class Model<D> {
  ...
  restore() {
    if (!this.seeds || typeof this.seeds !== "function") {
      throw new Error("There is no initial state to restore database.")
    }
    this.db.data = this.seeds()
    this.db.write()
    return this
